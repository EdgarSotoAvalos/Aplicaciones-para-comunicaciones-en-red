'''
    SERVIDOR QUE ACEPTA MULTIPLES CLIENTES UTILIZANDO CLIENTES PARA JUGAR AL MEMORAMA
    VERSION 1.0
...
DIF= None
'''Funcion Main'''
HOST = "8.40.1.86"  # Direccion de la interfaz de loopback estándar (localhost)
PORT = 65432  # Puerto que usa el cliente  (los puertos sin provilegios son > 1023)
buffer_size = 1024
# Función para manejar clientes utilizando hilos
def manejar_cliente(Client_conn, Client_addr):
    global DIF,matriz,ciega,fils,cols,juego
    # Verificar si ya se estableció la dificultad
    if DIF is None:
        # Si no se ha establecido, permitir que el cliente seleccione la dificultad
        Client_conn.sendall(bytes(
            'Bienvenido al Juego de Memoria\nSelecciona la dificultad:\n1. 4x4\n2. 6x6\nIngresa tu seleccion (1 o 2):',
            'utf-8'))
        ...
        while True:
            ...
            if choice == '1':
                ...
                #Client_conn.sendall(bytes("Dificultad establecida a 4x4\n", 'utf-8'))
                break
            elif choice == '2':
                fils, cols = 6, 6
                DIF = "6x6"
                matriz = crear_matriz(fils, cols)
                ciega = matriz_ciega(fils, cols)
                #Client_conn.sendall(bytes("Dificultad establecida a 6x6\n", 'utf-8'))
                break
            else:
                Client_conn.sendall(bytes("Entrada invalida.Ingresa de nuevo la dificultad\n", 'utf-8'))
    else:
        # Si ya se estableció la dificultad, informarla al cliente
        Client_conn.sendall(bytes(f"Dificultad actual: {DIF}\n", 'utf-8'))
    ...
    while True:
        ...
        if juego == (fils * cols) / 2:
            ...
            Client_conn.sendall(bytes(ciega_str + "\n", 'utf-8'))
            break
    # Cerramos la conexión cuando el juego termina
    Client_conn.close()
...
def juego_inicio():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as servidor_socket:
        ...
        while True:
            # Esperamos a que llegue una nueva conexión
            print("Esperando a recibir conexiones... ")
            cliente_socket, client_addr = servidor_socket.accept()
            print("Conexión establecida con", client_addr)
            listaConexiones.append(cliente_socket)
            gestion_conexiones(listaConexiones)
            print("\n")
            # Creamos un nuevo hilo para manejar al cliente
            hilo_cliente = threading.Thread(target=manejar_cliente, args=(cliente_socket, client_addr))
            hilo_cliente.start()
